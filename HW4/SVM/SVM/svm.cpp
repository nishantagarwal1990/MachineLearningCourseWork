...
int SVM(double rate, double tradeoff,vector<double> &weights, table &trainset,bool test,int *t_val)
{
    int mistake = 0;
    double sumw = 0.0;
    double rate_t = 0.0;
   ...
   // cout<<test_val++<<endl;
    for(int i =0; i<trainset.size();++i)
    {
        ...
        if (!test){
            ...
            if(sumw <= 1)
            {
                rate_t = ((double)rate/(double)(1+(double)((rate*(*t_val))/tradeoff)));
                //cout<< rate<<endl;
                for(int k = 0; k<weights.size();++k)
                {
                    weights[k] = weights[k]-(double)(rate_t*(weights[k]-(double)(tradeoff*trainset[i][k+1]* trainset[i][0])));
                }
                ++(*t_val);
                ...
                for(int k = 0; k<weights.size();++k)
                {
                    weights[k] = weights[k]-(double)(rate_t*(weights[k]));
                }
                ++(*t_val);
            }
        }
...
int main(int argc, char**argv){
   ...
   // double maxaccsimple = -1.0;
    ...
    for(int z = 0; z < folders.size(); ++z)
    {
        ...
        generatetable(testdata, trainfilename,d);
